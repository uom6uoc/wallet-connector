...
const WALLET_INFO = {
  ...
  name: 'MetaMask',
  rdns: 'io.metamask',
  uuid: '8bbb01ca-9b80-4a3c-9cc0-bee90569ed62',
};
...
export const WindowProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    if (typeof window.ethereum !== 'undefined') {
      ...
      const provider = new ethers.providers.Web3Provider(window.ethereum); // etherjs v5
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        if (accounts?.[0]) {
          setSelectedWalletUuid(WALLET_INFO.uuid);
          setSelectedAccountByWalletUuid((currentAccounts) => ({
            ...currentAccounts,
            [WALLET_INFO.uuid]: accounts[0],
          }));

          console.log('[window] : Selected Account', accounts[0]);
        }
        ...
        console.error('[window]Failed to connect to MetaMask', error);
      }
    } else {
    ...
    async (walletUuid: string) => {
      try {
        if (walletUuid === selectedWalletUuid) {
          ...
          console.log('[window] disConnectWallet:', wallet);
        }
      } catch (error) {
